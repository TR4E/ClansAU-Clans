...
public class InviteCommand extends ClanSubCommand implements EventContainer<ClanInviteEvent> {
    ...
    public void onEvent(final ClanInviteEvent event) {
        ...
        UtilLogger.log(Clans.class, "Clans", "Requests", UtilString.format("%s has invited %s to join %s", player.getName(), targetPlayer.getName(), this.getModule().getManager().getClanFullName(clan, null)));
    }
}