...
import me.trae.clans.clan.events.command.MemberDemoteEvent;
...
public class DemoteCommand extends ClanSubCommand implements EventContainer<MemberDemoteEvent> {
    ...
    public void onEvent(final MemberDemoteEvent event) {
        ...
        if (player.getUniqueId().equals(target.getUUID())) {
            UtilMessage.simpleMessage(player, "Clans", "You demoted yourself to <green><var></green>.", Collections.singletonList(member.getRole().getName()));
        } else {
